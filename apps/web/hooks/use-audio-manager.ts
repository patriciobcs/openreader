...
import { AudioChunk, PlaybackState, TTSProvider } from '@openreader/core/types';
import { getCurrentWordIndex, updateChunkWithTiming } from '@openreader/core/text-utils';
import {
  ...
  type WordTimestamp
} from '@openreader/core/audio-analysis';
import { convertCharacterTimingsToWords } from '@openreader/core/elevenlabs-utils';
    ...
    async (chunkIndex: number, shouldAutoPlay: boolean = false) => {
      ...
      try {
        ...
        if (wordTimings && wordTimings.length > 0) {
          ...
          updatedChunk.duration = wordTimings[wordTimings.length - 1].end;
