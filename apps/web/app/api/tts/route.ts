...
import type { ElevenLabsAlignment } from '@openreader/core/types';

// Simple in-memory cache for demo content
const ttsCache = new Map<string, { audio: Buffer; contentType: string; alignment?: ElevenLabsAlignment }>();
...
async function handleElevenLabsRequest(
  ...
  try {
    ...
    return NextResponse.json(
      { error: `ElevenLabs API error: ${message}` },
      { status: statusCode }
    );
  }
}
